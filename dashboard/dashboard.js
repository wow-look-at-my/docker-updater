...
function row(c) {
    ...
    return el("tr", { class: pending(c) ? "row-pending" : null }, nameCell, autoUpdateCell(c), stateCell(c), el("td", { class: uptime ? null : "up-na" }, uptime || "—"), restartsCell(c), el("td", { class: lastChecked === "—" ? "up-na" : null }, lastChecked), el("td", { class: lastPulled === "—" ? "up-na" : null }, lastPulled), upstreamCell(c));
}
...
function jumpToPending() {
    ...
    if (exitedSection && exitedSection.contains(first))
        exitedSection.open = true;
